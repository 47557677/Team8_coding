...
#读取数据
data=read.csv("F:/google下载/seat.csv",header=TRUE)#读取数据
...
spplot(subset(seat4,gpa.all>=bestgpa),col.regions=rw.colors(17))
...
for(p in 1:80){
  ...
  v=cd[p,][,3]
  for(i in 1:80){
    if(x==cd[i,][,1]&y+1==cd[i,][,2]){
      v=v+cd[i,][,3]
    }
    if(x-1==cd[i,][,1]&y==cd[i,][,2]){
      v=v+cd[i,][,3]
    }
    if(x+1==cd[i,][,1]&y-1==cd[i,][,2]){
      v=v+cd[i,][,3]
    }
    if(x+1==cd[i,][,1]&y==cd[i,][,2]){
      v=v+cd[i,][,3]
...
sp<-SpatialPointsDataFrame(cds,cd[4])
sp1<-as(sp,"SpatialPixelsDataFrame")#强制转化为SpatialPixelsDataFrame
rw.colors<-colorRampPalette(c("grey","red"))
spplot(sp1,col.regions=rw.colors(17))#绘制核密度图形
